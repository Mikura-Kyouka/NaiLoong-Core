...
class RegRenaming extends Module {
  ...
  // 申请寄存器
  when(ren_alloc_ready) {
    for (i <- 0 until 4) {
      when(io.from(i).valid && io.from(i).ready) {
        val new_freeHead = (freeHead + allocCount) % RegConfig.FREE_LIST_SIZE.U
  ...
  for (i <- 0 until 4) {
    when(io.commit_free_valid(i)) {
      val enqueueIndex = (freeTail + i.U) % RegConfig.FREE_LIST_SIZE.U
      freeList(enqueueIndex) := io.commit_free_preg(i)
    }
  }
  freeTail := (freeTail + commitFreeCount) % RegConfig.FREE_LIST_SIZE.U
