...
class ROB extends Module {
  ...
  val robEntries = RegInit(VecInit(Seq.fill(ROB_SIZE)(0.U.asTypeOf(new RobEntry))))
  ...
  when(io.rob_alloc_ready && (numDispatch =/= 0.U)) {
    for (i <- 0 until 4) {
      when(io.disp_valid(i)) {
        val allocIdx = (tail + i.U)(log2Ceil(ROB_SIZE)-1, 0)
        robEntries(allocIdx) := 0.U.asTypeOf(new RobEntry)
        robEntries(allocIdx).valid      := true.B
        robEntries(allocIdx).complete   := false.B
        robEntries(allocIdx).areg       := io.disp_info(i).areg
        robEntries(allocIdx).preg       := io.disp_info(i).preg
        robEntries(allocIdx).opreg      := io.disp_info(i).opreg
        robEntries(allocIdx).pc         := io.disp_info(i).pc
        robEntries(allocIdx).exception  := false.B
        robEntries(allocIdx).excpCode   := 0.U
        robEntries(allocIdx).instType   := io.disp_info(i).instType
        robEntries(allocIdx).checkpoint := io.disp_info(i).checkpoint
      }
    }
  ...
  when(io.flush_valid) {
    ...
    val newCount = Wire(UInt((log2Ceil(ROB_SIZE)+1).W))
    when(flushIdx >= head) {
      newCount := flushIdx - head
    } .otherwise {
      newCount := flushIdx + ROB_SIZE.U - head
    }
    
    val oldCount = count
    for (i <- 0 until ROB_SIZE) {
      when(i.U >= newCount && i.U < oldCount) {
        val idx = (head + i.U)(log2Ceil(ROB_SIZE)-1, 0)
        robEntries(idx).valid := false.B
      }
    }
