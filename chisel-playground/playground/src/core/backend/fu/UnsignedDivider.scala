...
class UnsignedDividerBlackBox extends BlackBox {
  override val desiredName = "UnsignedDivider"
...
class UnsignedDivider extends Module {
  ...
  switch(state) {
    ...
    is(calc) {
      // 模拟除法 latency：第35周期完成计算
      when(latencyCounter === 1.U) {
        // 执行无符号除法
        when(divisor === 0.U) {
          quotient := "hFFFFFFFF".U
          remainder := dividend
        }.otherwise {
          quotient := dividend / divisor
          val absRemainder = (dividend.abs % divisor.abs).asUInt
