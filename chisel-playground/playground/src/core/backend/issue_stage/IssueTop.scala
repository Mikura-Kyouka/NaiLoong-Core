...
class IssueTop extends Module {
  ...
  val alu1rs = Module(new UnorderIssueQueue(SIZE = UNORDER_QUEUE_SIZE, MAX_CNT = 2))
  val alu2rs = Module(new UnorderIssueQueue(SIZE = UNORDER_QUEUE_SIZE, MAX_CNT = 2))
  val mdurs  = Module(new UnorderIssueQueue(SIZE = MDU_QUEUE_SIZE, MAX_CNT = 4))
